...
class TelethonClient(BaseBotClient):
    ...
    async def initialize(self, is_bot: bool = True) -> None:
        ...
        try:
            ...
            bot_logger.info(f"✅ Telethon client initialized ({'bot' if is_bot else 'user'})")
        except Exception as err:
            bot_logger.error(f"❌ Failed to initialize Telethon client: {err}")
            raise
    ...
    async def start(self) -> None:
        ...
        while self._is_running:
            try:
                ...
                bot_logger.warning(f"⏳ Flood wait: {wait_time}s")
                await asyncio.sleep(wait_time)

            except (RPCError, ConnectionError) as err:
                bot_logger.error(f"❌ Connection error: {err}")
                await asyncio.sleep(self._reconnect_delay)

            except Exception as err:
                bot_logger.error(f"❌ Failed to start client: {err}")
                await asyncio.sleep(self._reconnect_delay)
        ...
        if self._client:
            try:
                if self._client.is_connected():
                    ...
                    bot_logger.info("⛔ Telethon client stopped")
            except Exception as err:
                bot_logger.error(f"❌ Error disconnecting: {err}")
    ...
    async def get_status(self) -> dict[str, Any]:
        ...
        if connected and self._client:
            try:
                ...
                bot_logger.debug(f"Could not get user info: {err}")
    ...
    async def send_message(self, chat_id: int, text: str, **kwargs: Any) -> dict[str, Any]:
        ...
        try:
            ...
            bot_logger.warning(f"⏳ Flood wait sending message: {err.seconds}s")
            return {
                "success": False,
            ...
            bot_logger.error(f"❌ RPC error sending message: {err}")
            return {
                "success": False,
                "error": str(err),
                "chat_id": chat_id,
            ...
            bot_logger.error(f"❌ Failed to send message: {err}")
            return {
                "success": False,
                "error": str(err),
                "chat_id": chat_id,
    ...
    async def get_me(self) -> dict[str, Any]:
        ...
        try:
            ...
            bot_logger.error(f"❌ RPC error getting user: {err}")
            raise
        except Exception as err:
            bot_logger.error(f"❌ Failed to get user: {err}")
            raise
    ...
    async def add_event_handler(self, handler_func: Callable, event_type: EventBuilder | None = None) -> None:
        ...
        try:
            ...
            bot_logger.debug("✅ Event handler added")
        except Exception as err:
            bot_logger.error(f"❌ Failed to add event handler: {err}")
            raise
    ...
    async def remove_event_handler(self, handler_func: Callable) -> bool:
        ...
        try:
            ...
            bot_logger.debug("✅ Event handler removed")
            return True
        except Exception as err:
            bot_logger.error(f"❌ Failed to remove event handler: {err}")
            return False
    ...
    async def get_dialogs(self, limit: int = 100) -> list[dict[str, Any]]:
        ...
        try:
            ...
            bot_logger.debug(f"✅ Retrieved {len(dialogs)} dialogs")
        except Exception as err:
            bot_logger.error(f"❌ Failed to get dialogs: {err}")
