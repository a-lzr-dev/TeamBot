...
class AiogramClient(BaseTelegramClient):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            tg_logger.info("✅ Aiogram client initialized successfully")
        except Exception as err:
            tg_logger.error(f"❌ Failed to initialize Aiogram client: {err}")
            if self._bot:
    ...
    async def start(self) -> None:
        ...
        tg_logger.debug("✅ Aiogram client started (polling will be handled by manager)")
            ...
            try:
                ...
                try:
                    ...
                    if "flood" in error_str or "retry after" in error_str:
                        tg_logger.debug(f"ℹ️ Bot close skipped due to flood: {err}")
                    else:
                        raise
                except Exception as err:
                    tg_logger.debug(f"ℹ️ Bot close error (ignored): {err}")

                # Закрываем сессию
                if hasattr(self._bot, "session") and self._bot.session:
                    await self._bot.session.close()
                    tg_logger.debug("⛔ Aiogram session closed")

            except Exception as err:
                tg_logger.error(f"❌ Failed to close aiogram bot: {err}")

            self._bot = None

        self._session = None
        tg_logger.info("⛔ Aiogram client stopped")
    ...
    async def send_message(self, chat_id: int, text: str, **kwargs: Any) -> dict[str, Any]:
        ...
        try:
            ...
            tg_logger.error(f"❌ Telegram API error sending message to {chat_id}: {err}")
            return {
                "success": False,
                "error": str(err),
                "chat_id": chat_id,
            ...
            tg_logger.error(f"❌ Failed to send message to {chat_id}: {err}")
            return {
                "success": False,
                "error": str(err),
                "chat_id": chat_id,
    ...
    async def get_me(self) -> dict[str, Any]:
        ...
        try:
            ...
            tg_logger.error(f"❌ Failed to get bot info: {err}")
            raise
        except Exception as err:
            tg_logger.error(f"❌ Unexpected error getting bot info: {err}")
            raise
