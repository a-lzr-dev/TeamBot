...
async def handle_new_message(event: events.NewMessage.Event, client: TelegramClient) -> None:
    ...
    tg_logger.debug(f"📩 New message from {message.sender_id} in chat {message.chat_id}")
    ...
    async with db_manager.get_session() as session:
        try:
            ...
            if existing_chat is None:
                ...
                tg_logger.debug(f"✅ Chat {message.chat_id} already exists")

        except Exception as err:
            tg_logger.error(f"❌ Failed to create/find chat: {err}", exc_info=True)
        ...
        try:
            ...
            if is_valid_user:
                try:
                    ...
                    tg_logger.warning(f"⚠️ Failed to save sender {user_id}: {err}")
                    user_id = None
            else:
            ...
            chat_message = ChatMessageModel(
                FID=message.id,
                FK_Chat=message.chat_id,
                FK_User=user_id,
                FK_MessageType=MessageType.USER_REQUEST,
                FK_ContentType=content_type,
                FText=message_text,
                FSource=MessageSource.USER,
                FDateSent=message.date.replace(tzinfo=None) if message.date else None,
                FFlagReply=bool(message.reply_to),
                FFlagForwarded=bool(message.forward),
                FFlagDeleted=False,
                FDateDeleted=None,
                FK_DeletedByMessage=None,
                FDeletedByType=None,
            ...
            tg_logger.debug(f"✅ Message {message.id} saved to database with lifetime {lifetime_seconds}s")

        except Exception as err:
            tg_logger.error(f"❌ Failed to save message: {err}", exc_info=True)
            await session.rollback()
...
async def handle_edited_message(event: events.MessageEdited.Event, _: TelegramClient) -> None:
    ...
    tg_logger.debug(f"📝 Message {message.id} edited in chat {message.chat_id}")
        ...
        try:
            ...
            if db_message:
                ...
                if db_message.FFlagDeleted:
                    tg_logger.debug(f"⏭️ Message {message.id} is deleted, skipping edit")
                    return
                ...
                tg_logger.warning(f"⚠️ Message {message.id} not found in database for edit")

        except Exception as err:
            tg_logger.error(f"❌ Failed to handle edited message: {err}", exc_info=True)
            await session.rollback()
...
async def handle_deleted_message(event: events.MessageDeleted.Event) -> None:
    ...
    tg_logger.info(f"🗑️ {len(event.deleted_ids)} messages deleted in chat {event.chat_id}")
    ...
    async with db_manager.get_session() as session:
        try:
            ...
            tg_logger.error(f"❌ Failed to mark deleted messages: {err}", exc_info=True)
            await session.rollback()
